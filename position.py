...
import config
from exceptions import InvalidFENException, InvalidPositionException
...
class Position:
    ...
    def from_db(cls, record: "PositionRecord") -> "Position":
        ...
        obj = cls()
        # record.bitboard arrives already unsigned — databases.fetch_position()
        # performs the signed/unsigned conversion at the SQLite boundary.
        bitboard = record.bitboard
    ...
    def to_db_format(self) -> tuple[int, bytes]:
        """Encodes this position into the (bitboard, pieces) database format.

        The bitboard is a 64-bit unsigned integer where bit N is set if a
        piece occupies square N (a1=bit0, h8=bit63).

        The pieces BLOB stores one nibble (4 bits) per piece, in the same
        order as the set bits in the bitboard (ascending square index). Two
        nibbles are packed into each byte: the first piece occupies the high
        nibble, the second occupies the low nibble. If the piece count is
        odd, the last byte has the final nibble in the high position and
        zeros in the low nibble.

        Note:
            The bitboard returned here is always unsigned. The signed/
            unsigned conversion required by SQLite's storage format is
            applied later, at the database boundary
            (see databases.insert_position).

        Returns:
            A tuple of (bitboard, pieces) where bitboard is an unsigned
            64-bit integer and pieces is a bytes object.

        Raises:
            InvalidPositionException: If this position's squares dict
                contains an invalid square index or nibble value.
        """
        bitboard: int = 0
        nibble_list: list[int] = []

        # self.squares is a dict square:piece where only squares with pieces are keys.
        for sq in sorted(self.squares):
            if not (0 <= sq <= 63):
                raise InvalidPositionException(
                    f"Invalid square index: {sq}", square=sq
                )

            nibble = self.squares[sq]
...
def _pack_nibbles(nibbles: list[int]) -> bytes:
    ...
    See config.py for which nibble represents which piece.

    Private helper for Position.to_db_format(); not part of the public API.
